...
@Repository("XPress:XStaffs:XStaffEventLogDAO")
public class XStaffEventLogDAOImp extends BaseDAO implements XStaffEventLogDAO {
	@PersistenceContext
	private EntityManager em;

	@Override
	public List<EventLogWrapper<StaffEventLog>> findAll(ControllerData metadata) {
		final CriteriaBuilder cb = em.getCriteriaBuilder();
		final CriteriaQuery<EventLogWrapper> select = cb.createQuery(EventLogWrapper.class);
		final Root<StaffEventLog> from = select.from(StaffEventLog.class);
		final Join<StaffEventLog, Lea> lea = from.join(JOIN_LEA, JoinType.LEFT);
		...
		return (List<EventLogWrapper<StaffEventLog>>) q.getResultList();
	}

	@Override
	public List<EventLogWrapper<StaffEventLog>> findAllByLeaRefId(ControllerData metadata, String refId) {
		final CriteriaBuilder cb = em.getCriteriaBuilder();
		final CriteriaQuery<EventLogWrapper> select = cb.createQuery(EventLogWrapper.class);
		final Root<StaffEventLog> from = select.from(StaffEventLog.class);
		final Join<StaffEventLog, Lea> lea = from.join(JOIN_LEA, JoinType.LEFT);
	...
	@Override
	public List<EventLogWrapper<StaffEventLog>> findAllBySchoolRefId(ControllerData metadata, String refId) {
		final CriteriaBuilder cb = em.getCriteriaBuilder();
		final CriteriaQuery<EventLogWrapper> select = cb.createQuery(EventLogWrapper.class);
		final Root<StaffEventLog> from = select.from(StaffEventLog.class);
		final Join<StaffEventLog, Lea> lea = from.join(JOIN_LEA, JoinType.LEFT);
		final Join<StaffEventLog, Staff> staff = from.join(JOIN_STAFF, JoinType.LEFT);
	...
	@Override
	public List<EventLogWrapper<StaffEventLog>> findAllByCourseRefId(ControllerData metadata, String refId) {
		final CriteriaBuilder cb = em.getCriteriaBuilder();
		final CriteriaQuery<EventLogWrapper> select = cb.createQuery(EventLogWrapper.class);
		final Root<StaffEventLog> from = select.from(StaffEventLog.class);
		final Join<StaffEventLog, Lea> lea = from.join(JOIN_LEA, JoinType.LEFT);
		final Join<StaffEventLog, Staff> staff = from.join(JOIN_STAFF, JoinType.LEFT);
		final SetJoin<Staff, StaffCourseSection> staffCourseSections = (SetJoin<Staff, StaffCourseSection>) staff.<Staff, StaffCourseSection>join(JOIN_STAFF_COURSE_SECTIONS, JoinType.LEFT);
	...
	@Override
	public List<EventLogWrapper<StaffEventLog>> findAllByRosterRefId(ControllerData metadata, String refId) {
		final CriteriaBuilder cb = em.getCriteriaBuilder();
		final CriteriaQuery<EventLogWrapper> select = cb.createQuery(EventLogWrapper.class);
		final Root<StaffEventLog> from = select.from(StaffEventLog.class);
		final Join<StaffEventLog, Lea> lea = from.join(JOIN_LEA, JoinType.LEFT);
		...
		select.where(
			cb.and(
				cb.greaterThanOrEqualTo(from.get(EVENT_TIMESTAMP), metadata.getChangesSinceLocalDateTime()),
				lea.get(ControllerData.LEA_LOCAL_ID).in(metadata.getApplication().getApp().getDistrictLocalIds()),
				cb.equal(courseSection.get(COURSE_SECTION_REF_ID), refId)
			)
		);
		select.orderBy(cb.asc(from.get(EVENT_TIMESTAMP)));
	...
	@Override
	public List<EventLogWrapper<StaffEventLog>> findAllByStudentRefId(ControllerData metadata, String refId) {
		final CriteriaBuilder cb = em.getCriteriaBuilder();
		final CriteriaQuery<EventLogWrapper> select = cb.createQuery(EventLogWrapper.class);
		final Root<StaffEventLog> from = select.from(StaffEventLog.class);
		final Join<StaffEventLog, Lea> lea = from.join(JOIN_LEA, JoinType.LEFT);
